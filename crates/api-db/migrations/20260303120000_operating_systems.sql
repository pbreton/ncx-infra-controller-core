...
-- Supports all variants: inline iPXE, image (reference to os_images), and template-based iPXE OS definition.
-- Relationship and tables similar to bare-metal-manager-rest; instance refers to an OS ID and can override some values.

CREATE TYPE operating_system_type AS ENUM ('ipxe', 'image', 'ipxe_os_definition');
...
CREATE TABLE IF NOT EXISTS operating_systems (
    ...
    type                    operating_system_type NOT NULL,
    ...
    -- Variant: image (reference to os_images)
    os_image_id             uuid REFERENCES os_images(id),
